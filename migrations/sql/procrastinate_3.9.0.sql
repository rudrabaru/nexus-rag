-- Procrastinate 3.9.0 schema, copied verbatim from procrastinate.schema.SchemaManager.get_schema().
-- Frozen here so migration 0002 always creates the same objects, whatever version is installed.
-- Upgrading Procrastinate: add a migration that runs the library's matching files in procrastinate/sql/migrations/.
